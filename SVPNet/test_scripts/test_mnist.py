...
def main(model, device, args):
    ...
    if args.data_source == 'moving_mnist_ori':
        from data.moving_mnist_live import load_test_dataset
        test_dataset = load_test_dataset(args.data_source, args.data_path, args.seq_len, args.horizon)
    elif args.data_source.startswith('moving_mnist'):
        from data.moving_mnist_adv_live import load_test_dataset
        test_dataset = load_test_dataset(args.data_source, args.data_path, args.seq_len, args.horizon)
    ...
    with torch.no_grad():
        ...
        for it, data in enumerate(data_loader):
            ...
            if num_samples < 100:
                ground_truth.append((frames * 255).astype(np.uint8))
                prediction.append((out * 255).astype(np.uint8))
            ...
            for i in range(args.horizon):
                ...
                ssim = 0
                for b in range(batch_size):
                    ssim += compare_ssim((frame_i[b, 0] * 255).astype(np.uint8), 
                                         (out_i[b, 0] * 255).astype(np.uint8))
    ...
    np.savez_compressed(
        os.path.join('{}/{}_prediction.npz'.format(args.result_path, args.data_source)),
