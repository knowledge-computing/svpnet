...
def main(model, device, args):
    ...
    from data.turbulent import load_test_dataset
    test_dataset = load_test_dataset(args.data_path, args.seq_len, args.horizon)
    data_loader = DataLoader(test_dataset, batch_size=16, shuffle=False, drop_last=True)
    ...
    with torch.no_grad():
        ...
        for it, data in enumerate(data_loader):
            ...
            if num_samples < 100:
                ground_truth.append((frames * 100).astype(np.uint8))
                prediction.append((out * 100).astype(np.uint8))
